...
BEGIN
    ...
    -- Create indexes
    EXECUTE IMMEDIATE 'CREATE INDEX idx_addresses_employee_id ON addresses(employee_id)';
    EXECUTE IMMEDIATE 'CREATE INDEX idx_addresses_post_code ON addresses(post_code)';
    EXECUTE IMMEDIATE 'CREATE INDEX idx_addresses_country ON addresses(country)';

    -- Add comments
    EXECUTE IMMEDIATE 'COMMENT ON TABLE addresses IS ''Standardized address information for employees with data quality tracking''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.address_id IS ''Unique identifier for the address''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.employee_id IS ''Reference to the employee''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.address_type IS ''Type of address (HOME, WORK, etc.)''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.address_line1 IS ''Original address line 1''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.address_line2 IS ''Original address line 2''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.address_line3 IS ''Original address line 3''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.main_building_number IS ''Extracted main building number''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.sub_building_number IS ''Extracted sub-building number (apt, floor, etc.)''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.street_name IS ''Extracted street name''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.town_and_city IS ''Town and city name''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.post_code IS ''Postal or ZIP code''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.country IS ''Country name''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.full_address IS ''Complete formatted address string''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.is_valid IS ''Data quality validation flag''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.validation_errors IS ''Description of any validation errors''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.is_active IS ''Active status of the address''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.created_date IS ''Record creation timestamp''';
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN addresses.modified_date IS ''Record last modification timestamp''';

    -- Create trigger for modified_date
    EXECUTE IMMEDIATE 'CREATE OR REPLACE TRIGGER trg_addresses_modify
    ...
    EXECUTE IMMEDIATE 'CREATE OR REPLACE PROCEDURE populate_addresses IS
        v_error_message VARCHAR2(4000);
        v_error_code NUMBER;
    ...
    -- Add comment
    EXECUTE IMMEDIATE 'COMMENT ON PROCEDURE populate_addresses IS ''Populates the addresses table with standardized address information from the locations table''';
